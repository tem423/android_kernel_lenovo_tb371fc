...
static int aud_clk_enable(const struct audio_clock_attr *clk)
{
	...
	if (clk->clk_prepare) {
		...
		if (ret) {
			pr_info("%s(), clk_enable %s fail, ret %d\n",
			       __func__, clk->name, ret);
			return -EPERM;
		}
	} else {
		pr_info("%s(), clk %s, not prepared\n", __func__, clk->name);
		return -EINVAL;
	}
...
void AudDrv_AUDINTBUS_Sel(int parentidx)
{
	...
	if (!aud_clks[CLOCK_MUX_AUDIOINTBUS].clk_prepare ||
		...
		pr_info("%s(), clk_prepare = false\n", __func__);
		goto EXIT;
	}

	clksys_set_reg(AUDIO_CLK_CFG_5_CLR, 0x3, 0x3);
	clksys_set_reg(AUDIO_CLK_CFG_5_SET, parentidx, 0x3);
	pr_debug("%s(), parentidx = %d, CLK_CFG_5 = 0x%08x\r\n",
		 __func__, parentidx, clksys_get_reg(AUDIO_CLK_CFG_5));
...
static int apll1_mux_setting(bool enable)
{
	...
	if (enable) {
		...
		if (ret) {
			pr_info("%s clk_set_parent %s-%s fail %d\n",
...
void AudDrv_Clk_On(void)
{
	...
	if (Aud_AFE_Clk_cntr == 1) {
		...
		if (aud_clks[CLOCK_SCP_SYS_AUD].clk_status) {
			...
			if (ret) {
				pr_info("%s [CCF]Aud clk_prepare_enable %s fail\n",
					__func__,
...
void AudDrv_Clk_Off(void)
{
	...
	if (Aud_AFE_Clk_cntr == 0) {
		...
		pr_info("!! %s(), Aud_AFE_Clk_cntr<0 (%d)\n", __func__,
			Aud_AFE_Clk_cntr);
		Aud_AFE_Clk_cntr = 0;
	}
	mutex_unlock(&auddrv_clk_mutex);
...
void AudDrv_ADC_Clk_On(void)
{
	...
	if (Aud_ADC_Clk_cntr == 0) {
		if (aud_clks[CLOCK_ADC].clk_prepare) {
			...
			if (ret) {
				pr_info("%s [CCF]Aud enable_clock %s fail",
					__func__, aud_clks[CLOCK_ADC].name);
				goto EXIT;
			}
		} else {
			pr_info("%s [CCF]clk_prepare error %s fail",
				__func__, aud_clks[CLOCK_ADC].name);
			goto EXIT;
		}
	}
...
void AudDrv_ADC_Clk_Off(void)
{
	...
	if (Aud_ADC_Clk_cntr < 0) {
		pr_info("%s(), Aud_ADC_Clk_cntr %d < 0\n",
...
void AudDrv_ADC2_Clk_Off(void)
{
	...
	if (Aud_ADC2_Clk_cntr < 0) {
		pr_info("%s(), Aud_ADC2_Clk_cntr %d < 0\n",
...
void AudDrv_I2S_Clk_Off(void)
{
	...
	if (Aud_I2S_Clk_cntr == 0) {
		...
		pr_info("!! %s(), Aud_I2S_Clk_cntr<0 (%d)\n", __func__,
			Aud_I2S_Clk_cntr);
		Aud_I2S_Clk_cntr = 0;
	}
	spin_unlock_irqrestore(&auddrv_Clk_lock, flags);
...
void AudDrv_TDM_Clk_Off(void)
{
	...
	if (Aud_TDM_Clk_cntr == 0) {
		...
		pr_info("!! %s(), Aud_TDM_Clk_cntr<0 (%d)\n",
...
void AudDrv_APLL1Tuner_Clk_Off(void)
{
	...
	if (Aud_APLL1_Tuner_cntr == 0) {
		...
		pr_info("!! %s, Aud_APLL1_Tuner_cntr<0 (%d)\n",
...
void SetckSel(unsigned int I2snum, unsigned int SampleRate)
{
	...
	case Soc_Aud_I2S3:
		...
		break;
	default:
		pr_info("%s(), not support I2snum %u\n", __func__, I2snum);
		break;
	}
...
unsigned int SetCLkMclk(unsigned int I2snum, unsigned int SampleRate)
{
	...
	case Soc_Aud_I2S3:
		...
		break;
	default:
		pr_info("[AudioWarn] %s(): I2snum = %d not recognized\n",
			__func__, I2snum);
		break;
	}
