/*
 * Huaqin Hardware Info Driver - Modified for Qualcomm SM8250 (Snapdragon 870)
 */
...
static ssize_t show_fingerprint_sn(struct device *dev, struct device_attribute *attr, char *buf)
{
    if (NULL != hwinfo_data.fingerprint_sn) {
        return sprintf(buf, "fingerprint_sn :%02x%02x%02x%02x%02x%02x\n",
                       hwinfo_data.fingerprint_sn[0], hwinfo_data.fingerprint_sn[1],
                       hwinfo_data.fingerprint_sn[2], hwinfo_data.fingerprint_sn[3],
                       hwinfo_data.fingerprint_sn[4], hwinfo_data.fingerprint_sn[5]);
    }
...
static ssize_t show_main_otp(struct device *dev, struct device_attribute *attr, char *buf)
{
    if (hw_info_main_otp.otp_valid) {
        return sprintf(buf, "main otp :Vendor 0x%0x, ModuleCode 0x%x, ModuleVer 0x%x, SW_ver 0x%x, Date %d-%d-%d, VcmVendor 0x%0x, VcmModule 0x%x \n",
                       hw_info_main_otp.vendor_id, hw_info_main_otp.module_code, hw_info_main_otp.module_ver,
                       hw_info_main_otp.sw_ver, hw_info_main_otp.year, hw_info_main_otp.month, hw_info_main_otp.day,
                       hw_info_main_otp.vcm_vendorid, hw_info_main_otp.vcm_moduleid);
    }
...
static ssize_t show_main2_otp(struct device *dev, struct device_attribute *attr, char *buf)
{
    if (hw_info_main2_otp.otp_valid) {
        return sprintf(buf, "main otp :Vendor 0x%0x, ModuleCode 0x%x, ModuleVer 0x%x, SW_ver 0x%x, Date %d-%d-%d, VcmVendor 0x%0x, VcmModule 0x%x \n",
                       hw_info_main2_otp.vendor_id, hw_info_main2_otp.module_code, hw_info_main2_otp.module_ver,
                       hw_info_main2_otp.sw_ver, hw_info_main2_otp.year, hw_info_main2_otp.month, hw_info_main2_otp.day,
                       hw_info_main2_otp.vcm_vendorid, hw_info_main2_otp.vcm_moduleid);
    }
    return sprintf(buf, "main otp :No Valid OTP\n");
}
...
static ssize_t show_main3_otp(struct device *dev, struct device_attribute *attr, char *buf)
{
    if (hw_info_main3_otp.otp_valid) {
        return sprintf(buf, "main otp :Vendor 0x%0x, ModuleCode 0x%x, ModuleVer 0x%x, SW_ver 0x%x, Date %d-%d-%d, VcmVendor 0x%0x, VcmModule 0x%x \n",
                       hw_info_main3_otp.vendor_id, hw_info_main3_otp.module_code, hw_info_main3_otp.module_ver,
                       hw_info_main3_otp.sw_ver, hw_info_main3_otp.year, hw_info_main3_otp.month, hw_info_main3_otp.day,
                       hw_info_main3_otp.vcm_vendorid, hw_info_main3_otp.vcm_moduleid);
    }
    return sprintf(buf, "main otp :No Valid OTP\n");
}
...
static ssize_t show_sub_otp(struct device *dev, struct device_attribute *attr, char *buf)
{
    if (hw_info_sub_otp.otp_valid) {
        return sprintf(buf, "sub otp :Vendor 0x%0x, ModuleCode 0x%x, ModuleVer 0x%x, SW_ver 0x%x, Date %d-%d-%d, VcmVendor 0x%0x, VcmModule 0x%0x \n",
                       hw_info_sub_otp.vendor_id, hw_info_sub_otp.module_code, hw_info_sub_otp.module_ver,
                       hw_info_sub_otp.sw_ver, hw_info_sub_otp.year, hw_info_sub_otp.month, hw_info_sub_otp.day,
                       hw_info_sub_otp.vcm_vendorid, hw_info_sub_otp.vcm_moduleid);
    }
...
/* 骁龙870: 从cmdline读取stage，不依赖IIO ADC */
...
static int hw_info_parse_dt(struct device_node *np)
{
    ...
    if (np) {
        ...
        if (type_gpio_pin0 >= 0 && type_gpio_pin1 >= 0 && type_gpio_pin2 >= 0) {
            current_type_value = (gpio_get_value(type_gpio_pin0) << 0) |
                                 (gpio_get_value(type_gpio_pin1) << 1) |
                                 (gpio_get_value(type_gpio_pin2) << 2);
            hwinfo_debug("[HWINFO] current_type_value=%d\n", current_type_value);
        } else {
            current_type_value = 2; // 默认值
            hwinfo_debug("[HWINFO] GPIO not found, use default type=2\n");
        }
...
static int HardwareInfo_driver_probe(struct platform_device *pdev)
{
    ...
    if (pcb_name && strcmp(pcb_name, "UNKNOWN") == 0) {
        hwinfo_debug("[HWINFO] UNKNOWN pcb name\n");
    }
