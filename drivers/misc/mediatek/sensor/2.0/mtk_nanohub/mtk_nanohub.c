...
struct mtk_nanohub_device {
	...
	//Spruce code for OSPURCET-789 by huweifeng at 2023.01.14 start
	int32_t light_config_data[5];
	int32_t rgbw_config_data[2];
	//Spruce code for OSPURCET-789 by huweifeng at 2023.01.14 end
		...
		enum CUST_ACTION action, void *data)
{
	...
	case ID_SAR:
		...
		break;
	/*Spruce code for OSPURCET-467 by dengbing at 2022.11.25 start*/
	case ID_RGBW:
		req.set_cust_req.sensorType = ID_RGBW;
		req.set_cust_req.action = SENSOR_HUB_SET_CUST;
		switch (action) {
		case CUST_ACTION_GET_SENSOR_INFO:
			req.set_cust_req.getInfo.action =
		...
		break;
	/*Spruce code for OSPURCET-467 by dengbing at 2022.11.25 end*/
	case ID_OIS:
...
static void mtk_nanohub_restoring_config(void)
{
	...
	//Spruce code for OSPURCET-789 by huweifeng at 2023.01.14 start
	length = sizeof(device->rgbw_config_data);
	data = vzalloc(length);
	if (data) {
		spin_lock(&config_data_lock);
		memcpy(data, device->rgbw_config_data, length);
		spin_unlock(&config_data_lock);
		mtk_nanohub_cfg_to_hub(ID_RGBW, data, length);
		vfree(data);
	}
	//Spruce code for OSPURCET-789 by huweifeng at 2023.01.14 end
		...
		int sensor_type, void *data, uint8_t length)
{
	...
	//Spruce code for OSPURCET-789 by huweifeng at 2023.01.14 start
	case ID_RGBW:
		if (sizeof(device->rgbw_config_data) < length)
			length = sizeof(device->rgbw_config_data);
		spin_lock(&config_data_lock);
		memcpy(device->rgbw_config_data, data, length);
		spin_unlock(&config_data_lock);
		break;
	//Spruce code for OSPURCET-789 by huweifeng at 2023.01.14 end
	case ID_PROXIMITY:
		...
		int sensor_type, struct custom_cmd *cust_cmd)
{
	...
	if (cust_action == CUST_CMD_CALI) {
		...
		case SENSOR_TYPE_RGBW:
			if (sizeof(cust_cmd->data) <
					sizeof(device->rgbw_config_data))
				return -EINVAL;
			cust_cmd->rx_len = sizeof(device->rgbw_config_data);
			spin_lock(&config_data_lock);
			memcpy(cust_cmd->data, device->rgbw_config_data,
					sizeof(device->rgbw_config_data));
			spin_unlock(&config_data_lock);
			break;
		//Spruce code for OSPURCET-789 by huweifeng at 2023.01.14 end
...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		...
		case ID_GYROSCOPE:
			...
			break;
		/*Spruce code for OSPURCET-261 by yujx7 at 2022.12.01 start*/
		case ID_SAR:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			...
			event.word[3] = data->data[3];
			break;
		/*Spruce code for OSPURCET-261 by yujx7 at 2022.12.01 end*/
		}
	} else {
		event.timestamp = data->time_stamp;
		event.sensor_type = id_to_type(data->sensor_type);
...
static ssize_t sar_show(struct device_driver *ddri,
		char *buf)
{
	struct sensorInfo_t devinfo;
	memset(&devinfo,0,sizeof(devinfo));
	
	if (mtk_nanohub_set_cmd_to_hub(ID_SAR,CUST_ACTION_GET_SENSOR_INFO, &devinfo) < 0) {
		pr_err("mtk_nanohub_set_cmd_to_hub fail,(ID: %d),(action: %d) \n", ID_SAR,CUST_ACTION_GET_SENSOR_INFO);
		return 0;
	} else {
...
static ssize_t msensor_show(struct device_driver *ddri,
		char *buf)
{
	struct sensorInfo_t devinfo;
	memset(&devinfo,0,sizeof(devinfo));
	
	if (mtk_nanohub_set_cmd_to_hub(ID_MAGNETIC_FIELD,CUST_ACTION_GET_SENSOR_INFO, &devinfo) < 0) {
		pr_err("mtk_nanohub_set_cmd_to_hub fail,(ID: %d),(action: %d) \n", ID_MAGNETIC_FIELD,CUST_ACTION_GET_SENSOR_INFO);
		return 0;
	} else {
		return sprintf(buf,"%s\n",devinfo.name);
	}
...
static ssize_t a_gsensor_show(struct device_driver *ddri,
		char *buf)
{
	struct sensorInfo_t devinfo;
	memset(&devinfo,0,sizeof(devinfo));
	
	if (mtk_nanohub_set_cmd_to_hub(ID_ACCELEROMETER,CUST_ACTION_GET_SENSOR_INFO, &devinfo) < 0) {
...
static ssize_t rgb_show(struct device_driver *ddri,
		char *buf)
{
	struct sensorInfo_t devinfo;
	memset(&devinfo,0,sizeof(devinfo));
	
	if (mtk_nanohub_set_cmd_to_hub(ID_RGBW,CUST_ACTION_GET_SENSOR_INFO, &devinfo) < 0) {
		pr_err("mtk_nanohub_set_cmd_to_hub fail,(ID: %d),(action: %d) \n", ID_RGBW,CUST_ACTION_GET_SENSOR_INFO);
		return 0;
...
static DRIVER_ATTR_RO(rgb);
static DRIVER_ATTR_RO(sar);
static DRIVER_ATTR_RO(msensor);
static DRIVER_ATTR_RO(a_gsensor);

static struct driver_attribute *mtk_nanohub_attrs[] = {
	&driver_attr_trace,
	&driver_attr_sar,
	&driver_attr_msensor,
	&driver_attr_a_gsensor,
	&driver_attr_rgb,
};
