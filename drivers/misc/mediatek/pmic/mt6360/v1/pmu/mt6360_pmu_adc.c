...
struct mt6360_pmu_adc_info {
	...
	/*Spruce code for OSPURCET-40 by liugong2 at 20221201 start*/
	/*ntc array*/
	unsigned int *ntc_array;
	int ts_len;
	/*Spruce code for OSPURCET-40 by liugong2 at 20221201 end*/
};
...
/*Spruce code for OSPURCET-40 by liugong2 at 20221201 start*/
static int get_ts_table_index(int *table, int table_len, int val)
{
	int index = 0;
	int i = 0;
				      ...
				      int chan_idx, int *val)
{
	...
	case TS_CHANNEL:
		/*Spruce code for OSPURCET-40 by liugong2 at 20221201 start*/
		*val = ((*val * 1250) * 1000)/ 18;
		*val = get_ts_table_index(info->ntc_array, info->ts_len, *val) - 40;
		/*Spruce code for OSPURCET-40 by liugong2 at 20221201 end*/
		break;
...
/*Spruce code for OSPURCET-40 by liugong2 at 20221201 start*/
static int mt6360_tsadc_parse_dt_data(struct device *dev, struct mt6360_pmu_adc_info *mapi)
{
	int ret = 0;
	int length = 0;
	struct device_node *np = dev->of_node;

	ret = of_property_count_elems_of_size(np, "ntc-table", sizeof(u32));
	if(ret < 0){
		dev_err(dev, "ts adc get size of ntc array failed!!!");
		return -EINVAL;
	}
	length = ret;
	mapi->ts_len = length;//164

	mapi->ntc_array = devm_kzalloc(dev, sizeof(int) * (length + 1), GFP_KERNEL);
	if(!mapi->ntc_array){
		ret = -ENOMEM;
		dev_err(dev, "ntc array kzalloc failed!!!");
		return ret;
	}

	ret = of_property_read_u32_array(np, "ntc-table", (u32 *)mapi->ntc_array, length);
	if(ret){
		dev_err(dev, "ts adc read ntc array failed!!!");
		return -EINVAL;
	}

	dev_info(dev, "ts ntc table parse successful!!!");
	return ret;
}
/*Spruce code for OSPURCET-40 by liugong2 at 20221201 end*/
...
static int mt6360_pmu_adc_probe(struct platform_device *pdev)
{
	...
	/*Spruce code for OSPURCET-40 by liugong2 at 20221201 start*/
	if(use_dt){
		ret = mt6360_tsadc_parse_dt_data(mpai->dev,  mpai);
		if (ret < 0) {
			dev_err(&pdev->dev, "parse ntc dt fail\n");
			return ret;
		}
	}
	/*Spruce code for OSPURCET-40 by liugong2 at 20221201 end*/
