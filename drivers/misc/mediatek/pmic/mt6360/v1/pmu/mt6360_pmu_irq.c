...
/* Spruce code for OSPURCET-1414 by tangsf2 at 2023.02.25 start */
#include <linux/workqueue.h>

struct work_data {
        struct work_struct mt6360_workqueue;
        struct mt6360_pmu_info *data;
};

struct work_data mt6360_work_data;

void mt6360_workqueue_func(struct work_struct *work)
{
        struct work_data *pdata = (struct work_data *)container_of(work, struct work_data, mt6360_workqueue);
	struct mt6360_pmu_info *mpi = pdata->data;
/* Spruce code for OSPURCET-1414 by tangsf2 at 2023.02.25 end */
...
{
        mt6360_work_data.data = data;
        schedule_work(&mt6360_work_data.mt6360_workqueue);
/* Spruce code for OSPURCET-1414 by tangsf2 at 2023.02.25 end */
	return IRQ_HANDLED;
}
...
static int mt6360_pmu_gpio_irq_init(struct mt6360_pmu_info *mpi)
{
	...
	/* Spruce code for OSPURCET-1414 by tangsf2 at 2023.02.25 start */
        mt6360_work_data.data = NULL;
        INIT_WORK(&mt6360_work_data.mt6360_workqueue, mt6360_workqueue_func);
	/* Spruce code for OSPURCET-1414 by tangsf2 at 2023.02.25 end */
