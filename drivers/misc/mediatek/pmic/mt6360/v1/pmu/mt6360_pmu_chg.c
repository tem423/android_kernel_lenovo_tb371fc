...
/* Spruce code for OSPURCET-256 by zhangjb18 at 2022.11.24 start */
#include <v1/hardinfo_charger.h>
/* Spruce code for OSPURCET-256 by zhangjb18 at 2022.11.24 end */
...
static int mt6360_set_pep20_efficiency_table(struct charger_device *chg_dev)
{
	...
	chg_mgr->pe2.profile[0].vchr = 10000000;
	chg_mgr->pe2.profile[1].vchr = 10000000;
	chg_mgr->pe2.profile[2].vchr = 10000000;
	chg_mgr->pe2.profile[3].vchr = 10000000;
	chg_mgr->pe2.profile[4].vchr = 10000000;
	chg_mgr->pe2.profile[5].vchr = 10000000;
	chg_mgr->pe2.profile[6].vchr = 10000000;
	chg_mgr->pe2.profile[7].vchr = 10000000;
	chg_mgr->pe2.profile[8].vchr = 10000000;
	chg_mgr->pe2.profile[9].vchr = 10000000;
	return 0;
}
...
static int mt6360_set_shipping_mode(struct mt6360_pmu_chg_info *mpci)
{
	...
	if (!mpci->cp_psy){
		mpci->cp_psy = power_supply_get_by_name("sgm41600-standalone");
		if (!mpci->cp_psy) {
			dev_notice(mpci->dev,  "%s: not sgm,get sc854x psy!!!\n", __func__);
			mpci->cp_psy = power_supply_get_by_name("sc854x-standalone");
		}
	}
	if(!mpci->cp_psy){
		dev_notice(mpci->dev,
	...
	propval.intval = 1;
	ret = power_supply_set_property(mpci->cp_psy, POWER_SUPPLY_PROP_ADC_DIS, &propval);
...
#endif /* CONFIG_MT6360_PMU_CHARGER_TYPE_DETECT && !CONFIG_TCPC_CLASS */
/* Spruce code for OSPURCET-256 by zhangjb18 at 2022.11.24 start */
	hardinfo_set_vendor_chginfo(HRADINFO_MAIN_BUCK_INFO, MT6360);
/* Spruce code for OSPURCET-256 by zhangjb18 at 2022.11.24 end */
