		...
		unsigned long event, void *data)
{
	...
	struct power_supply *cp_psy = power_supply_get_by_name("sgm41600-standalone");
	if (!cp_psy){
		dev_info(dev, "%s: extcon not sgm41600,get sc854x psy!!!\n", __func__);
		cp_psy = power_supply_get_by_name("sc854x-standalone");
	}
	if (!cp_psy)
		dev_info(dev, "%s: extcon get power supply failed\n", __func__);
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		/* Spruce code for OSPURCET-1748 by zhongxc7 at 2023.03.16 start */
		propval.intval = !(noti->typec_state.new_state);
		dev_info(dev, "intval=%d\n", propval.intval);
		power_supply_set_property(cp_psy, POWER_SUPPLY_PROP_ADC_DIS, &propval);
		/* Spruce code for OSPURCET-1748 by zhongxc7 at 2023.03.16 end */
