...
#ifdef VCODEC_DVFS_V2
static void vcodec_venc_pmqos(struct codec_job *enc_cur_job)
{
	int target_freq;
	u64 target_freq_64;
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	ret = snprintf(vcodec_qos_request.owner,
		sizeof(vcodec_qos_request.owner) - 1, "vdec_bw");
	if(ret < 0)
		pr_debug("vcodec_qos_request.owner failed %d\n", ret);
	ret = snprintf(vcodec_qos_request2.owner,
		sizeof(vcodec_qos_request2.owner) - 1, "venc_bw");
	if(ret < 0)
		pr_debug("vcodec_qos_request2.owner failed %d\n", ret);
	ret = snprintf(vcodec_qos_request_f.owner,
		sizeof(vcodec_qos_request_f.owner) - 1, "vdec_freq");
	if(ret < 0)
		pr_debug("vcodec_qos_request_f.owner failed %d\n", ret);
	ret = snprintf(vcodec_qos_request_f2.owner,
		sizeof(vcodec_qos_request_f2.owner) - 1, "venc_freq");
	if(ret < 0)
		pr_debug("vcodec_qos_request_f2.owner failed %d\n", ret);
#endif
...
static const struct of_device_id vcodec_of_match[] = {
	{ .compatible = "mediatek,vcodec_dec", },
	{/* sentinel */}
};
