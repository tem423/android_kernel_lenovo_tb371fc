...
int search_HWLockSlot_ByTID(unsigned long ulpa, unsigned int curr_tid)
{
	...
	for (i = 0; i < MULTI_INST_NUM; i++) {
		if (oal_hw_context[i].u4VCodecThreadNum !=
			...
			for (j = 0; j <
				oal_hw_context[i].u4VCodecThreadNum; j++) {
	...
	unsigned int *a_prIndex)
{
	...
	for (i = 0; i < a_prThreadID.u4VCodecThreadNum; i++) {
		pr_debug("[VCODEC][%s] VCodecThreadNum = %d, VCodecThreadID = %d\n",
		     __func__, a_prThreadID.u4VCodecThreadNum,
		     a_prThreadID.u4VCodecThreadID[i]);
	}
	...
	for (i = 0; i < MULTI_INST_NUM; i++) {
		if (oal_hw_context[i].u4VCodecThreadNum !=
			...
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum; j++) {
				for (k = 0; k < a_prThreadID.u4VCodecThreadNum; k++) {
					if (oal_hw_context[i].u4VCodecThreadID[j] ==
					    a_prThreadID.u4VCodecThreadID[k]) {
						pr_debug("[VCODEC][%s]\n",
							__func__);
	...
	for (i = 0; i < MULTI_INST_NUM; i++) {
		if (oal_hw_context[i].u4VCodecThreadNum ==
			...
			oal_hw_context[i].u4VCodecThreadNum =
				a_prThreadID.u4VCodecThreadNum;
			for (j = 0; j < a_prThreadID.u4VCodecThreadNum; j++) {
				oal_hw_context[i].u4VCodecThreadID[j] =
				    a_prThreadID.u4VCodecThreadID[j];
			    ...
			    unsigned int *a_pu4ThreadID)
{
	...
	for (u4I = 0; u4I < MULTI_INST_NUM_x_10; u4I++) {
		if ((grNCMemoryList[u4I].ulKVA == -1L)
			...
			grNCMemoryList[u4I].u4VCodecThreadNum = a_u4ThreadNum;
			for (u4J = 0; u4J <
				grNCMemoryList[u4I].u4VCodecThreadNum; u4J++) {
				grNCMemoryList[u4I].u4VCodecThreadID[u4J] =
				    *(a_pu4ThreadID + u4J);
				pr_debug("[VCODEC][%s] VCodecThreadNum = %d, VCodecThreadID = %d\n",
				__func__,
				grNCMemoryList[u4I].u4VCodecThreadNum,
				grNCMemoryList[u4I].u4VCodecThreadID[u4J]);
			}
...
void Force_Free_NonCacheMemoryList(unsigned int a_u4Tid)
{
	...
	for (u4I = 0; u4I < MULTI_INST_NUM_x_10; u4I++) {
		if (grNCMemoryList[u4I].u4VCodecThreadNum
			== VCODEC_THREAD_MAX_NUM)
			continue;
		for (u4J = 0; u4J <
			grNCMemoryList[u4I].u4VCodecThreadNum; u4J++) {
			if (grNCMemoryList[u4I].u4VCodecThreadID[u4J]
				!= a_u4Tid)
				continue;
			pr_debug
				("[VCODEC][WARNING] %s\n", __func__);
			...
			for (u4K = 0; u4K <
				...
				grNCMemoryList[u4I].u4VCodecThreadID[u4K] =
					0xffffffff;
			}
			grNCMemoryList[u4I].ulKVA = -1L;
			grNCMemoryList[u4I].ulKPA = -1L;
