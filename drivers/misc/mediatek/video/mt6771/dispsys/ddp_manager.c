...
static int acquire_mutex(enum DDP_SCENARIO_ENUM scenario)
{
	...
	ASSERT(scenario >= DDP_SCENARIO_PRIMARY_DISP && scenario < DDP_SCENARIO_MAX);
		      ...
		      struct disp_ddp_path_config *config, void *cmdq_handle)
{
	...
	DISP_LOG_V("path config ovl %d,rdma %d,wdma %d,dst %d on path handle %p scenario %s\n",
...
int dpmgr_check_status(disp_path_handle dp_handle)
{
	...
	phandle = kmalloc(sizeof(struct ddp_path_handle), GFP_ATOMIC);
	if (IS_ERR_OR_NULL(phandle)) {
		DDPERR("%s:%d alloc path handle fail! phandle=%p\n",
	...
	memcpy(phandle, dp_handle, sizeof(struct ddp_path_handle));
	list = ddp_get_scenario_list(phandle->scenario);
	m_num = ddp_get_module_num(phandle->scenario);
