...
extern struct m4u_client_t *g_client;
#endif
static int primary_display_free_fb_buf(void);
#endif
...
static int primary_display_free_fb_buf(void)
{
	...
	if (!is_fb_freed) {
#ifdef MTKFB_M4U_SUPPORT
		ret = m4u_dealloc_mva(g_client, DISP_M4U_PORT_DISP_OVL0, pgc->framebuffer_mva);
		if (ret)
			pr_info("m4u_dealloc_mva returns fail: %d\n", ret);
#endif
		va_start = pgc->framebuffer_va;
		va_end = pgc->framebuffer_va + (unsigned long)vramsize;
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (!is_fb_freed && !ovl_is_config_fb) {
			...
			DISPMSG(
				"[%d/%d] ovl_is_config_fb=%d, id=%d/%d, 0x%p/%d, fb=0x%p/%d\n",
				i, cfg->input_layer_num, ovl_is_config_fb, ovl_cfg->layer,
				ovl_cfg->layer_en, (void *)ovl_cfg->addr, layer_size,
...
static int primary_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	if (pgc->state == DISP_SLEPT) {
		...
		if (cfg->present_fence_idx != (unsigned int)-1)
			mtkfb_release_present_timeline_fence(cfg->session_id);
