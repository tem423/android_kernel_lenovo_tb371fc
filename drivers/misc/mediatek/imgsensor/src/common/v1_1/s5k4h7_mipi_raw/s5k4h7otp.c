...
bool S5K4H7_update_awb(unsigned char page)
{
	...
	/*Spruce code for OSPURCET-603 by wanglc13 at 22/12/08 start*/
	golden_rg = read_cmos_sensor_8(0x0A15)
		| ((read_cmos_sensor_8(0x0A18) & 0xC0) >> 6);
	golden_bg = read_cmos_sensor_8(0x0A16)
		| ((read_cmos_sensor_8(0x0A18) & 0x30) >> 4);

	unit_rg = read_cmos_sensor_8(0x0A11) | ((read_cmos_sensor_8(0x0A14) & 0xC0) >> 6);
	unit_bg = read_cmos_sensor_8(0x0A12) | ((read_cmos_sensor_8(0x0A14) & 0x30) >> 4);
	/*Spruce code for OSPURCET-603 by wanglc13 at 22/12/08 end*/
	LOG_INF(
...
bool s5k4h7_mifawb_datacheck(unsigned char * ucMIFAWB_data)
{
	...
	for(i = 1; i < 31; i++)
	{
		...
		/*Tulip code for TULIP-448 by guomin at 2022/5/11 start*/
		//LOG_INF("MIF&AWB data[%d] = 0x%02x\n",i,ucMIFAWB_data[i]);
		/*Tulip code for TULIP-448 by guomin at 2022/5/11 end*/
	}
	uiChecksum_cal = uiChecksum_cal % 255 + 1;
	uiChecksum_read = ucMIFAWB_data[31];

	/*TULIP code for TULIP-847 by guomin at 20220622 start*/
	if(uiChecksum_cal != uiChecksum_read)
	{
		LOG_INF("MIF&AWB read checksum 0x%02x not match calculated checksum 0x%02x!",uiChecksum_read,uiChecksum_cal);
		return 0;
	} else {
		LOG_INF("JOJO MIF&AWB checksum successful: read checksum 0x%02x same with calculated checksum 0x%02x!",uiChecksum_read,uiChecksum_cal);
	}
...
bool s5k4h7_lsc_datacheck(unsigned int group)
{
	...
	if(uiChecksum_cal != uiChecksum_read)
	{
		LOG_INF("LSC read checksum 0x%02x not match calculated checksum 0x%02x!",uiChecksum_read,uiChecksum_cal);
		return 0;
	}
	else
	{
	...
	for(group = 0;group < S5K4H7_MIFAWB_GROUP_CNT;group++)
	{
		...
		case 0x40:
			...
			for(i = 1; i < 64; i++)
			{
				...
				ucMIFAWB_addr[i] = i;
				//printk("jojo s5k4h7 No.%d: %02x", i,ucMIFAWB_data[i]);
			}
			/*TULIP code for TULIP-847 by guomin at 20220622 start*/
