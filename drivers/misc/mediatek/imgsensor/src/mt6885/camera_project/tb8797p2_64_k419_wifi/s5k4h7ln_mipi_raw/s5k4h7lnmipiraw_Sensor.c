...
#define PFX "s5k4h7ln_camera_sensor"
#define LOG_INF(format, args...) pr_info(PFX "[%s] " format, __func__, ##args)


static DEFINE_SPINLOCK(imgsensor_drv_lock);

#include "s5k4h7lnmipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7LN_SENSOR_ID,	/*S5K4H7_SENSOR_ID = 0x487B */
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{3264, 2448,   0,   0, 3264, 2448, 3264, 2448, 0000, 0000, 3264, 2448, 0, 0, 3264, 2448},	// normal video
	{3264, 2448, 352, 504, 2560, 1440, 1280,  720, 0000, 0000, 1280,  720, 0, 0, 1280,  720},	// high speed
...
static void write_shutter(kal_uint32 shutter)
{
	...
	if (shutter >= (imgsensor_info.max_frame_length - imgsensor_info.margin))
	{
		...
		else if (shutter <= 303674 ){ //4s
			LOG_INF("4s mode");
			write_cmos_sensor_8(0x0340, 0x32);
			write_cmos_sensor_8(0x0341, 0x17);
			write_cmos_sensor_8(0x0202, 0x32);
			write_cmos_sensor_8(0x0203, 0x12);

			write_cmos_sensor_8(0x0342, 0xFF);
			write_cmos_sensor_8(0x0343, 0xFC);
		}
		else if (shutter <= 607348 ){ //8s
			LOG_INF("8s mode");
			write_cmos_sensor_8(0x0340, 0x84);
			write_cmos_sensor_8(0x0341, 0xA3);
			write_cmos_sensor_8(0x0202, 0x84);
			write_cmos_sensor_8(0x0203, 0x9E);
