...
static int mtktscharger_get_hw_temp(void)
{
	...
	/*Spruce code for OSPURCET-40 by liugong2 at 20221128 start*/
	int times = 1, Channel = g_RAP_ADC_channel; /* 6752=0(AUX_IN5_NTC) */
	/*Spruce code for OSPURCET-40 by liugong2 at 20221128 end*/
...
static int mtktscharger_register_thermal(void)
{
	...
	/* trips : trip 0~2 */
	thz_dev = mtk_thermal_zone_device_register("main_mtktscharger", num_trip,
...
static int mtktscharger_pdrv_probe(struct platform_device *pdev)
{
	...
	/*Spruce code for OSPURCET-40 by liugong2 at 20221202 start*/
	...
	if (ret) {
		mtktscharger_dprintk_always(
			"[%s] fail to get auxadc iio ch5: %d\n",
			__func__, ret);
		return ret;
	}
	/*Spruce code for OSPURCET-40 by liugong2 at 20221202 end*/
...
#ifdef CONFIG_OF
/*Spruce code for OSPURCET-40 by liugong2 at 20221202 start*/
static const struct of_device_id mt_thermistor_of_match3[2] = {
	{.compatible = "mediatek,mtboard-thermistor3",},
	{},
};
#endif

#define THERMAL_THERMISTOR_NAME    "mtboard-thermistor_main_chg"
/*Spruce code for OSPURCET-40 by liugong2 at 20221202 end*/
