...
#ifdef CONFIG_WAS4780_AUDIO_SWITCH
#include "../../../was4780.h"
#endif
...
#ifdef CONFIG_WAS4780_AUDIO_SWITCH
static struct notifier_block typec_nb;
static struct device_node *typec_node;
...
static inline void enable_accdet(u32 state_swctrl)
{
	...
	pmic_write_set(PMIC_ACCDET_CMP_PWM_EN_ADDR,0);
#endif
	/* enable ACCDET unit */
...
static unsigned int accdet_switch_manual_work(void)
{
	...
	pre_accdet_adc = pmic_get_auxadc_value(AUXADC_LIST_ACCDET);
	if (pre_accdet_adc > switch_mode_voltage) {
		pr_info("[accdet] 4-pole CTIA headset. no need swap.");
		was4780_switch_event(typec_node, WAS_EARPIECE_TYPE, CTIA);
	} else {
		/* CTIA/OMTP changes */
		was4780_switch_event(typec_node, WAS_MIC_GND_SWAP, 0);
		new_accdet_adc = pmic_get_auxadc_value(AUXADC_LIST_ACCDET);
		if (new_accdet_adc > switch_mode_voltage) {
			was4780_switch_event(typec_node, WAS_EARPIECE_TYPE, OMTP);
			pr_info("[accdet] 4-pole OMTP headset : %u", pre_accdet_adc);
		} else {
			was4780_switch_event(typec_node, WAS_EARPIECE_TYPE, THREE_POLE);
...
static int accdet_get_dts_data(void)
{
	...
	typec_node = of_parse_phandle(node, "was4780_audio_switch", 0);
	if (IS_ERR_OR_NULL(typec_node)) {
		pr_err("%s failed to parse audio switch node, please check", __func__);
		return -1;
	} else {
		pr_err("%s typec_node name: %s", __func__, typec_node->name);
	}
...
#ifdef CONFIG_WAS4780_AUDIO_SWITCH
	typec_nb.notifier_call = ext_typec_event_changed;
	typec_nb.priority = 0;
	ret = was4780_reg_notifier(&typec_nb, typec_node);
	if (ret) {
...
#ifdef CONFIG_WAS4780_AUDIO_SWITCH
err_register_noti:
	was4780_unreg_notifier(&typec_nb, typec_node);
#endif
