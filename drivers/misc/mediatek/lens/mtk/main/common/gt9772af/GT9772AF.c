...
static int gt9772_init(void)
{
	int i4RetValue = 0;
	char puSendCmd[3];
 /*Spruce code for OSPURCET-787 by mengchao3 at 20230113 start*/
	g_pstAF_I2Cclient->addr = AF_I2C_SLAVE_ADDR;
	g_pstAF_I2Cclient->addr = g_pstAF_I2Cclient->addr >> 1;
 /*Spruce code for OSPURCET-787 by mengchao3 at 20230113 end*/
	printk("[GT9772AF] gt9772_init - beg\n");
	puSendCmd[0] = 0xED;
	puSendCmd[1] = 0xAB;
...
int GT9772AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		...
		LOG_INF("Wait\n");
		s4AF_WriteReg(300);
		/*Spruce code for OSPURCET-3155 by zhangzp16 at 20230318 start*/
		msleep(3);
		s4AF_WriteReg(250);
		msleep(3);
		s4AF_WriteReg(150);
		...
		s4AF_WriteReg(100);
		msleep(3);
		/*Spruce code for OSPURCET-3155 by zhangzp16 at 20230318 end*/
	}
	if (*g_pAF_Opened) {
		LOG_INF("Free\n");
