...
static void smblib_notify_device_mode(struct smb_charger *chg, bool enable)
{
	...
	power_supply_set_property(ext_psy,POWER_SUPPLY_PROP_usb_device_ctl,&reta);
}
...
static void smblib_notify_usb_host(struct smb_charger *chg, bool enable)
{
	...
	if (enable) {
		smblib_dbg(chg, PR_OTG, "enabling VBUS by pmi632 not used in OTG mode\n");
	...
	//	smblib_notify_extcon_props(chg, EXTCON_USB_HOST);
		...
		smblib_dbg(chg, PR_OTG, "disabling VBUS by pmi632 not used in OTG mode\n");
		//rc = smblib_masked_write(chg, DCDC_CMD_OTG_REG,
		//			OTG_EN_BIT, 0);
		if (rc < 0) {
			smblib_err(chg,
...
#define HOT_FV_4100MV 0x13  /*JEITA_FVCOMP_HOT=(4390-4200)/10*/
...
out:
	pr_err( "Current temperatures: \tDIE_TEMP: %d,\tCONN_TEMP: %d,\tSMB_TEMP: %d,\tSKIN_TEMP: %d\nTHERMAL_STATUS: %d\n",
		...
		if (usb_current == -ETIMEDOUT) {
			...
			if (chg->connector_type ==
				...
				rc = vote(chg->usb_icl_votable,
					...
					is_dock_charger() ? DOCK_CURRENT_UA : DCP_CURRENT_UA);
				if (rc < 0)
					return rc;
			}
		} else {
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pd_active) {
		 ...
		 * pd_current_max
		 */
		vote(chg->usb_icl_votable, PD_VOTER, true, USBIN_100MA*5);
		pr_err("========================@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@=====ddddd=======add fot test====\r\n");
...
irqreturn_t chg_state_change_irq_handler(int irq, void *data)
{
	...
	u8 stat;
	int rc;
	printk("==============leon=========chg_state_change_irq_handlerr=================\r\n");
...
irqreturn_t batt_temp_changed_irq_handler(int irq, void *data)
{
	...
	printk("==============leon=========batt_tmp_changed_irq_handler=================\r\n");
...
irqreturn_t batt_psy_changed_irq_handler(int irq, void *data)
{
	...
	printk("==============leon=========batt_psy_changed+irq_handler=================\r\n");
...
static void smblib_micro_usb_plugin(struct smb_charger *chg, bool vbus_rising)
{
	...
	if (!vbus_rising) {
		...
		//rc = typec_partner_register(chg,0);
		//if (rc < 0)
