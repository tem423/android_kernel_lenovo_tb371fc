...
struct mm8013_chip {
    ...
    char serial_num[8];      /* 存储电池ID字符串 */
};

static struct mm8013_chip *global_chip;

static int battery_id = 1;
...
static int mm8013_checkdevice(struct mm8013_chip *chip)
{
    ...
    {
        battery_id = 2;
        strcpy(chip->serial_num, "2");
    }
    else if (ret == 0x0101)
    {
        battery_id = 1;
        strcpy(chip->serial_num, "1");
    }
    else
    {
        printk("====unknown_battery==== \r\n");
        return -1;
    }
...
static enum power_supply_property mm8013_battery_props[] = {
    POWER_SUPPLY_PROP_CAPACITY,        /* 原 POWER_SUPPLY_PROP_batt_cap */
    POWER_SUPPLY_PROP_VOLTAGE_NOW,
    POWER_SUPPLY_PROP_CURRENT_NOW,
    POWER_SUPPLY_PROP_TEMP,
    POWER_SUPPLY_PROP_SERIAL_NUMBER,   /* 原 POWER_SUPPLY_PROP_battery_id */
};
                    ...
                    union power_supply_propval *val)
{
    ...
    switch (psp) {
    /* 修改2：使用对应的标准属性名 */
    case POWER_SUPPLY_PROP_CAPACITY:   /* 原 POWER_SUPPLY_PROP_batt_cap */
    ...
    case POWER_SUPPLY_PROP_TEMP:
        ...
        break;
    /* 修改3：SERIAL_NUMBER 使用 strval 返回字符串 */
    case POWER_SUPPLY_PROP_SERIAL_NUMBER:
        val->strval = chip->serial_num;
        break;
    default:
        return -EINVAL;
    }
            ...
            const struct i2c_device_id *id)
{
    ...
    if (!chip) {
        printk("can't alloc mm8013_chip\n");
        return -ENOMEM;
    }
    ...
    if (ret) {
        dev_err(&client->dev, "failed to access platform, no mm8013!\n");
        return -EPROBE_DEFER;
    }

    psy_cfg.drv_data = chip;
    chip->psy = power_supply_register(&client->dev, &mm8013_desc, &psy_cfg);
    if (IS_ERR(chip->psy)) {
