...
static int power_misc_routine_thread(void *arg)
{
	struct shutdown_controller *sdd = arg;
	int ret = 0;
	/* Spruce code for OSPURCET-2795 by xuyc12 at 2023.03.13 start */
	bool is_poweroff_charging  = is_kernel_power_off_charging();

	bm_err("[%s] is_poweroff_charging = %d\n", __func__, is_poweroff_charging);
	/* Spruce code for OSPURCET-2795 by xuyc12 at 2023.03.13 end */
	...
	while (1) {
		...
		// Spruce code for OSPURCET-2795 by xuyc12 at 2023.03.13
		if (sdd->overheat == true && (is_poweroff_charging != true)) {
			sdd->overheat = false;
