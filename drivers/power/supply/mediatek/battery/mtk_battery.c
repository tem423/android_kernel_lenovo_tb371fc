...
#define SPRUCE_CHARGE_FULL 10200000
	...
	union power_supply_propval *val)
{
	int ret = 0;
	int fgcurrent = 0;
/* Spruce code for OSPURCET-2570 by wumz4 at 2023.03.09 start */
	int bat_voltage = 0;
	static int low_battery_need_pwroff = 0;
/* Spruce code for OSPURCET-2570 by wumz4 at 2023.03.09 end */
...
/* Spruce code for OSPURCET-2570 by wumz4 at 2023.03.09 start */
		ret = mm8013_get_info(POWER_SUPPLY_PROP_VOLTAGE_NOW, &bat_voltage);
		if(ret < 0) {
			pr_err("fail to get bat_voltage\n");
		}
/* Spruce code for OSPURCET-2570 by wumz4 at 2023.03.09 end */
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 start */
		ret = mm8013_get_info(POWER_SUPPLY_PROP_CAPACITY, &val->intval);
		if(ret < 0) {
			val->intval = 50;
		} else {
			if (val->intval < 1)
...
/* Spruce code for OSPURCET-2570 by wumz4 at 2023.03.09 start */
/* Spruce code for OSPURCET-4397 by yeyz5 at 2023.03.21 start */
		if (val->intval <= 1 && bat_voltage < 3400 &&
			data->BAT_STATUS != POWER_SUPPLY_STATUS_CHARGING &&
			!is_kernel_power_off_charging()) {
			low_battery_need_pwroff++;
			pr_info("bat_voltage=%d,BAT_STATUS=%d,soc=%d\n",
				bat_voltage, data->BAT_STATUS, val->intval);
		} else if (val->intval <= 1 && bat_voltage < 3350 &&
			!is_kernel_power_off_charging()) {
			low_battery_need_pwroff++;
			pr_info("bat_voltage=%d,BAT_STATUS=%d,soc=%d\n",
				bat_voltage, data->BAT_STATUS, val->intval);
		}
		if(low_battery_need_pwroff >= 10){
			low_battery_need_pwroff = 0;
			pr_info("battery is low, shutdown!\n");
			mdelay(10);
			kernel_power_off();
		}
/* Spruce code for OSPURCET-4397 by yeyz5 at 2023.03.21 start */
/* Spruce code for OSPURCET-2570 by wumz4 at 2023.03.09 end */
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 start */
		ret = mm8013_get_info(POWER_SUPPLY_PROP_CURRENT_NOW, &val->intval);
		if(ret < 0) {
			val->intval = 500000;
		} else {
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.08 start */
		ret = mm8013_get_info(POWER_SUPPLY_PROP_CURRENT_AVG, &val->intval);
		if(ret < 0) {
			val->intval = 0;
		} else {
			val->intval = val->intval * 100;
		...
		ret = 0;
		/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.08 end */
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 start */
		val->intval = SPRUCE_CHARGE_FULL;
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 end */
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 start */
		ret = mm8013_get_info(POWER_SUPPLY_PROP_VOLTAGE_NOW, &val->intval);
		if(ret >= 0) {
	...
	case POWER_SUPPLY_PROP_TEMP:
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 start */
		ret = mm8013_get_info(POWER_SUPPLY_PROP_TEMP, &val->intval);
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 end */
		/* Spruce code for OSPURCET-33 by zhangjb18 at 2022.12.12 start */
		if(hardinfo_get_fake_temp() != HARDINFO_ERROR) { //use fake temp
			val->intval = hardinfo_get_fake_temp();
		}
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = mm8013_set_info(psp, val->intval);
		break;
	default:
		ret = -EINVAL;
		break;
