...
#include "./vendor/pd_policy_manager.h"
#include <v1/mtk_battery.h>
/* Spruce code for OSPURCET-1515 by tangsf2 at 2023.03.10 start */
extern int tcpc_norp_src;
/* Spruce code for OSPURCET-1515 by tangsf2 at 2023.03.10 end */
...
static void _disable_all_charging(struct charger_manager *info)
{
	...
	if (chg2_chip_enabled) {
		...
		pr_notice("%s ,trun off1 chg2_dev \n", __func__);
	}
	...
	if(mtk_is_pdc_pps_ready(info)){
		usbpd_pm_set_cp_status(false);
	}
...
extern bool bq_input_stop;
extern bool bq_charger_stop;
...
{
	struct charger_data *pdata, *pdata2, *pdata3;
	struct dual_switch_charging_alg_data *swchgalg = info->algorithm_data;
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.12.07 start */
	if (mtk_is_pdc_pps_ready(info)) {
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.12.07 end */
		...
		chr_err("type-C:%d current:%d\n",
			...
			adapter_dev_get_property(info->pd_adapter,
				TYPEC_RP_LEVEL));
/* Spruce code for OSPURCET-1515 by tangsf2 at 2023.03.10 start */
	} else if (tcpc_norp_src && !is_typec_adapter(info) && !mtk_pe20_get_is_connect(info) && info->chr_type != CHARGING_HOST) {
		pdata->input_current_limit = 500000;
		pdata->charging_current_limit = 500000;
		chr_err("tcpc norp: tcpc_norp_src=%d\n", tcpc_norp_src);
/* Spruce code for OSPURCET-1515 by tangsf2 at 2023.03.10 end */
		...
		if ((mtk_pe20_get_is_enable(info) &&
			...
			pdata2->input_current_limit
					= info->data.ac_charger_input_current*3/8;
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 start */
	ret = dual_swchg_get_gauge_info(POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbat);
	if(!!ret) {
		pr_notice("%s get vbat error\n", __func__);
	}
	...
	if (info->enable_sw_jeita) {
		...
		if (info->chr_type == STANDARD_HOST)
			...
			/* FFC CHARGE */
			if (mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info) ||
					mtk_is_pdc_pps_ready(info)) {
				switch (info->sw_jeita.sm) {
					case TEMP_ABOVE_T4: // [> 50]
					if(mtk_is_pdc_pps_ready(info)) {
						pdata3->charging_current_limit = info->data.jeita_temp_above_t4_cur;
					} else if(mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info)) {
							pdata->charging_current_limit =
								min(pdata->charging_current_limit, info->data.jeita_temp_above_t4_cur);
							pdata2->charging_current_limit =
								min(pdata2->charging_current_limit, info->data.jeita_temp_above_t4_cur);
					}
					break;
					case TEMP_T3_TO_T4: // [45-50C]
					if(mtk_is_pdc_pps_ready(info)) {
						if(vbat < FFC_4200MV) {
							if(info->step_45_50_charge == 1) {
								pdata3->charging_current_limit = 0;
							} else {
								pdata3->charging_current_limit = info->data.jeita_temp_t3_to_t4_cur;
							}
						} else {
							pdata3->charging_current_limit = 0;
							info->step_45_50_charge = 1;
						}
					} else if(mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info)) {
						if(vbat < FFC_4200MV) { // VBAT:[0-4200mv]
							if(info->step_45_50_charge == 1) {
								pdata->charging_current_limit = 0;
								pdata2->charging_current_limit = 0;
							} else {
								pdata->charging_current_limit =
									min(pdata->charging_current_limit, info->data.jeita_temp_t3_to_t4_cur*5/8);
								pdata2->charging_current_limit =
									min(pdata2->charging_current_limit, info->data.jeita_temp_t3_to_t4_cur*3/8);
							}
						} else { // VBAT:[> 4200mv]
							pdata->charging_current_limit = 0;
							pdata2->charging_current_limit = 0;
					...
					break;
					case TEMP_T2_TO_T3: // [15-45C]
					if(mtk_is_pdc_pps_ready(info)) {
						pdata3->charging_current_limit = info->data.jeita_temp_t2_to_t3_cur;
					} else if(mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info)) {
						pdata->charging_current_limit =
							min(pdata->charging_current_limit, info->data.jeita_temp_t2_to_t3_cur*5/8);
						pdata2->charging_current_limit =
							min(pdata2->charging_current_limit, info->data.jeita_temp_t2_to_t3_cur*3/8);
					}
					break;
					case TEMP_T0_TO_T1 ... TEMP_T1_TO_T2: // [0-15C]
					if(mtk_is_pdc_pps_ready(info)) {
						if(vbat < FFC_4200MV) {
							if(info->step_0_15_charge == 1) {
								pdata3->charging_current_limit = info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur;
							} else {
								pdata3->charging_current_limit = info->data.ffc_jeita_temp_t0_to_t2_0_4420mv_cur;
							}
						} else {
							info->step_0_15_charge = 1;
							pdata3->charging_current_limit = info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur;
						}
					} else if(mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info)) {
						if(vbat < FFC_4200MV) {
							if(info->step_0_15_charge == 1) {
								pdata->charging_current_limit =
									min(pdata->charging_current_limit, info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur*5/8);
								pdata2->charging_current_limit =
									min(pdata->charging_current_limit, info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur*3/8);
							} else {
								pdata->charging_current_limit =
									min(pdata->charging_current_limit, info->data.ffc_jeita_temp_t0_to_t2_0_4420mv_cur*5/8);
								pdata2->charging_current_limit =
									min(pdata2->charging_current_limit, info->data.ffc_jeita_temp_t0_to_t2_0_4420mv_cur*3/8);
							}
						} else {
							pdata->charging_current_limit =
								min(pdata->charging_current_limit, info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur*5/8);
					...
					break;
					case TEMP_BELOW_T0: // [0 <]
					if(mtk_is_pdc_pps_ready(info)) {
						pdata3->charging_current_limit = info->data.jeita_temp_below_t0_cur;
					} else if(mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info)) {
						pdata->charging_current_limit =
							min(pdata->charging_current_limit, info->data.jeita_temp_below_t0_cur);
						pdata2->charging_current_limit =
							min(pdata2->charging_current_limit, info->data.jeita_temp_below_t0_cur);
					}
					break;
				}
			/* STANDARD CHARGE */
				...
				case TEMP_ABOVE_T4:
					...
					break;
				case TEMP_T3_TO_T4:
					/* Spruce code for OSPURCET-2986 by gujy6 at 2023.03.13 start */
					if(vbat < FFC_4200MV) {
						if(info->step_45_50_charge == 1)
							pdata->charging_current_limit = 0;
						else
							pdata->charging_current_limit = info->data.jeita_temp_t3_to_t4_cur;
					} else {
						info->step_45_50_charge = 1;
						pdata->charging_current_limit = 0;
					}
					/* Spruce code for OSPURCET-2986 by gujy6 at 2023.03.13 end */
					break;
				case TEMP_T2_TO_T3:
					pdata->charging_current_limit =
						min(pdata->charging_current_limit, info->data.jeita_temp_t2_to_t3_cur);
					break;
				case TEMP_T0_TO_T1 ... TEMP_T1_TO_T2:
					/* Spruce code for OSPURCET-2986 by gujy6 at 2023.03.13 start */
					if(vbat < FFC_4200MV) {
						if(info->step_0_15_charge == 1)
							pdata->charging_current_limit = info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur;
						else
							pdata->charging_current_limit = info->data.ffc_jeita_temp_t0_to_t2_0_4420mv_cur;
					} else {
						info->step_0_15_charge = 1;
						pdata->charging_current_limit = info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur;
					}
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 end */
	...
	/* Spruce code for OSPURCET-605 by zhangjb18 at 2022.12.16 start */
	thermal_limit_current = hardinfo_get_thermal_limit();
	if(thermal_limit_current != -1) {
		if(usbpd_pm_get_cp_status()) {
			pdata3->thermal_charging_current_limit = thermal_limit_current;
		} else {
			/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.27 start */
			if (chg2_chip_enabled) {
				pdata->thermal_charging_current_limit = thermal_limit_current*5/8;
				pdata2->thermal_charging_current_limit = thermal_limit_current*3/8;
			} else {
				pdata->thermal_charging_current_limit = thermal_limit_current;
			}
			/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.27 start */
		}
	}
	...
	/* Spruce code for OSPURCET-605 by zhangjb18 at 2022.12.16 start */
	if (pdata3->thermal_charging_current_limit != -1) {
		if (pdata3->thermal_charging_current_limit <
		    pdata3->charging_current_limit)
			pdata3->charging_current_limit =
				pdata3->thermal_charging_current_limit;
	}
...
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.07 start */
	    !mtk_is_TA_support_pd_pps(info) && !mtk_is_pdc_pps_ready(info)) {
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.07 end */
	...
	if (hardinfo_get_bp()) {
		if (bq_input_stop) {
			pdata->input_current_limit = 0;
			pdata2->input_current_limit = 0;
			pdata3->input_current_limit = 0;
			pr_debug("[%s][%d]set input limit to 0\n", __func__, __LINE__);
		}
		if (bq_charger_stop) {
			pdata->charging_current_limit = 0;
			pdata2->charging_current_limit = 0;
			pdata3->charging_current_limit = 0;
			pr_debug("[%s][%d]set charge limit to 0\n", __func__, __LINE__);
		}
		pr_info("[%s][%d]bp is enbale,  bq_input_stop = %d, bq_charger_stop = %d\n",
				__func__, __LINE__, bq_input_stop, bq_charger_stop);
	}
	/* Spruce code for OSPURCET-1932 by xuyc12 at 2023.03.07 end */

	pr_notice("[%s]force:%d %d thermal:(%d %d,%d %d,%d)(%d %d %d)setting:(%d %d)(%d %d)(%d)[pps:%d][pe+2.0:%d][pd2.0:%d][cp:%d]",
		__func__,
		...
		_uA_to_mA(pdata3->thermal_charging_current_limit),
		...
		_uA_to_mA(pdata3->charging_current_limit),
		mtk_is_pdc_pps_ready(info),
		mtk_pe20_get_is_connect(info),
		mtk_pdc_check_charger(info),
		usbpd_pm_get_cp_status()
		);
/* Spruce code for OSPURCET-1515 by tangsf2 at 2023.03.10 start */
	chr_err("[%s] sw_jeita->sm:%d mater:charging_current_limit:%d, slave:charging_current_limit:%d, cp:charging_current_limit:%d, tcpc_norp_src=%d\n",
		__func__, info->sw_jeita.sm, _uA_to_mA(pdata->charging_current_limit),
		_uA_to_mA(pdata2->charging_current_limit), _uA_to_mA(pdata3->charging_current_limit), tcpc_norp_src);
/* Spruce code for OSPURCET-1515 by tangsf2 at 2023.03.10 end */
...
/* Spruce code for OSPURCET-463 by zhangjb18 at 2022.12.28 start */
	usbpd_pm_set_fcc(pdata3->charging_current_limit); //notify pps manager
...
/* Spruce code for OSPURCET-463 by zhangjb18 at 2022.12.28 end */
	if (((mtk_pe20_get_is_enable(info) && mtk_pe20_get_is_connect(info))
	    ...
	    || (mtk_pdc_check_charger(info) &&
		!dual_swchg_check_pd_leave(info))) && !mtk_is_pdc_pps_ready(info)) {
		if (chg2_chip_enabled) {
...
/* Spruce code for OSPURCET-450 by zhangjb18 at 2022.12.05 end */
	...
	if (pdata->thermal_input_current_limit == -1 &&
	    pdata->thermal_charging_current_limit > 1000000 &&
	    pdata2->thermal_input_current_limit == -1 &&
	    pdata2->thermal_charging_current_limit > 1000000) {
	...
	if (info->enable_sw_jeita)
		if (info->sw_jeita.cv != 0) {
			charger_dev_set_constant_voltage(info->chg1_dev,
							info->sw_jeita.cv);
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.12.01 start */
			charger_dev_set_constant_voltage(info->chg2_dev,
							info->sw_jeita.cv);
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.12.01 end */
			return;
		}
...
static void dual_swchg_turn_on_charging(struct charger_manager *info)
{
	...
	if (is_dual_charger_supported(info) == false) {
		chg2_enable = false;
		pr_notice("%s is_dual_charger_supported,chg2_enable=%d\n", __func__, chg2_enable);
	}
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 start */
			if(!mtk_is_pdc_pps_ready(info) && !mtk_pdc_check_charger(info)) {
...
/* Spruce code for OSPURCET-450 by zhangjb18 at 2022.12.05 start */
				...
				pr_notice("%s ,trun off2 chg2_dev \n", __func__);
/* Spruce code for OSPURCET-450 by zhangjb18 at 2022.12.05 end */
			...
			if (chg2_chip_enabled) {
				charger_dev_enable(info->chg2_dev, false);
				charger_dev_enable_chip(info->chg2_dev, false);
				pr_notice("%s ,trun off3 chg2_dev \n", __func__);
			}
		...
		if (chg2_chip_enabled) {
			charger_dev_enable(info->chg2_dev, false);
			charger_dev_enable_chip(info->chg2_dev, false);
			pr_notice("%s ,trun off4 chg2_dev \n", __func__);
		}
	}
...
{
	/* Spruce code for OSPURCET-2986 by gujy6 at 2023.03.13 start */
	info->step_0_15_charge = 0;
	info->step_45_50_charge = 0;
	/* Spruce code for OSPURCET-2986 by gujy6 at 2023.03.13 end */
...
static int mtk_dual_switch_chr_cc(struct charger_manager *info)
{
	...
	if (swchgalg->state != CHR_POSTCC && swchgalg->state != CHR_PE40_POSTCC
	    && chg2_en
/* Spruce code for OSPURCET-2188 by yeyz5 at 2023.02.28 start */
	    && (pdata->thermal_charging_current_limit > (info->data.dual_polling_ieoc + 500000) ||
/* Spruce code for OSPURCET-2188 by yeyz5 at 2023.02.28 end */
		pdata->thermal_charging_current_limit ==  -1)) {
		charger_dev_safety_check(info->chg1_dev,
					 info->data.dual_polling_ieoc);
	}
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 start */
	if(mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info) ||
					mtk_is_pdc_pps_ready(info)) {
		ret = dual_swchg_get_gauge_info(POWER_SUPPLY_PROP_CAPACITY, &soc);
		if(!!ret) {
			pr_info("not find capacity\n");
		}
		ret = dual_swchg_get_gauge_info(POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbat);
		if(!!ret) {
			pr_info("not find voltage\n");
		}
		ret = dual_swchg_get_gauge_info(POWER_SUPPLY_PROP_CURRENT_NOW, &cur);
		if(!!ret) {
			pr_info("not find current\n");
		}

		switch (info->sw_jeita.sm) {
		case TEMP_T3_TO_T4:
			ieoc = info->data.ffc_jeita_temp_t3_to_t4_cutoff_cur;
			break;
		case TEMP_T2_TO_T3:
			ieoc = info->data.ffc_jeita_temp_t2_to_t3_cutoff_cur;
			break;
		case TEMP_T0_TO_T1 ... TEMP_T1_TO_T2:
			ieoc = info->data.ffc_jeita_temp_t0_to_t2_cutoff_cur;
			break;
		default:
			break;
		}
		...
		if(soc == 100) {
			if(ieoc >= cur && vbat >= (info->sw_jeita.cv - 50000)) {
				// notcharging chg_dev1 chg_dev2
				charger_dev_enable(info->chg1_dev, false);
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.07 start */
				charger_dev_enable(info->chg2_dev, false);
				pr_notice("%s ,trun off5 chg2_dev \n", __func__);
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.07 end */
				chg_done = true;
			}
		}
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 start */
	if(mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info) ||
		...
		if(vbat <= info->data.ffc_vrecharge) {
			chg_done = false;
/* Spruce code for OSPURCET-450 by zhangjb18 at 2022.12.05 start */
				charger_dev_enable(info->chg1_dev, true);
/* Spruce code for OSPURCET-450 by zhangjb18 at 2022.12.05 end */
...
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.07 start */
		mtk_is_pdc_pps_ready(info) == false) {
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.07 end */
	...
	/* Spruce code for OSPURCET-605 by zhangjb18 at 2022.12.16 start */
	if(mtk_is_pdc_pps_ready(info) && mtk_pe20_get_is_connect(info)
	...
	if (event == CHARGER_DEV_NOTIFY_EOC) {
		...
		if (!chg_en || !chg2_chip_enabled) {
			...
			if (ichg2 - 500000 < ichg2_min) {
				...
				charger_dev_enable(info->chg2_dev, false);
				pr_notice("%s ,trun off6 chg2_dev \n", __func__);
				charger_dev_set_eoc_current(info->chg1_dev,
								500000);
				if (mtk_pe40_get_is_connect(info) == false)
					charger_dev_enable_termination(
