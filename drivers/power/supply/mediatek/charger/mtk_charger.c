...
/* Spruce code for OSPURCET-3205 by gujy6 at 2023.03.13 start */
static int last_temp;
/* Spruce code for OSPURCET-3205 by gujy6 at 2023.03.13 end */
...
int mtk_chr_get_type(void)
{
	...
	if(mtk_is_pdc_pps_ready(pinfo))
		return PD30_PPS_CHARGER;
	else if(mtk_pdc_check_charger(pinfo))
		return PD20_CHARGER;
	else if(mtk_pe20_get_is_connect(pinfo))
		return PEPLUS20_CHARGER;

	return pinfo->chr_type;
}
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 end */
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.27 start */
int mtk_chr_get_cv(void)
{
	if(IS_ERR_OR_NULL(pinfo))
		return 0;

	return pinfo->sw_jeita.cv;
}
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		sw_jeita->sm = TEMP_BELOW_T0;
		/* Spruce code for OSPURCET-3205 by gujy6 at 2023.03.13 start */
		if ((last_temp < info->data.temp_t0_thres) && (info->battery_temp == info->data.temp_t0_thres))
			chr_err("[SW_JEITA] warm up to 0 temp,start charging\n");
		else {
			sw_jeita->charging = false;
			chr_err("[SW_JEITA] cool down to 0 temp,keep discharging\n");
		}
		/* Spruce code for OSPURCET-3205 by gujy6 at 2023.03.13 end */
	}
...
/* Spruce code for OSPURCET-605 by zhangjb18 at 2022.12.16 start */
	switch (sw_jeita->sm)
	{
	case TEMP_BELOW_T0 ... TEMP_T2_TO_T3:
		sw_jeita->cv = info->data.ffc_jeita_temp_t0_to_t3_cv;
		break;
	case TEMP_T3_TO_T4 ... TEMP_ABOVE_T4:
		sw_jeita->cv = info->data.ffc_jeita_temp_above_t3_cv;
		break;
	default:
		sw_jeita->cv = info->data.ffc_battery_cv;
		break;
	}
	ret = bm_updata_func();
	if(ret > 0) {
		sw_jeita->cv = sw_jeita->cv > ret ? ret : sw_jeita->cv;
	}

	if (mtk_pe20_get_is_connect(info) || mtk_pdc_check_charger(info) ||
			mtk_is_pdc_pps_ready(info)) {
		info->data.ffc_vrecharge = sw_jeita->cv - 100000;
	} else {
		sw_jeita->cv -= 30000;
		info->data.ffc_vrecharge = 0; //clear fcc
	}
/* Spruce code for OSPURCET-605 by zhangjb18 at 2022.12.16 end */


	chr_err("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d ffc_rec:%d\n",
		sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
		sw_jeita->cv, info->data.ffc_vrecharge);
}
...
// workaround for mt6768 
	...
	if (count >= 3) {
		if (boot_mode == META_BOOT || boot_mode == ADVMETA_BOOT ||
			...
			/* Spruce code for OSPURCET-1410 by tangsf2 at 2023.02.24 start */
			kernel_power_off();
			/* Spruce code for OSPURCET-1410 by tangsf2 at 2023.02.24 end */
		}
	}
}
...
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 start */
	...
	/* sw jeita for ffc*/
	if (of_property_read_u32(np, "ffc_battery_cv", &val) >= 0)
		info->data.ffc_battery_cv = val;
	else {
		chr_err("use default FFC_BATTERY_CV:%d\n",
			FFC_BATTERY_CV);
		info->data.ffc_battery_cv = FFC_BATTERY_CV;
	}

	if (of_property_read_u32(np, "ffc_vrecharge", &val) >= 0)
		info->data.ffc_vrecharge = val;
	else {
		chr_err("use default FFC_VRECHAGRE:%d\n",
			FFC_VRECHARGE);
		info->data.ffc_vrecharge = FFC_VRECHARGE;
	}

	if (of_property_read_u32(np, "ffc_jeita_temp_above_t3_cv", &val) >= 0)
		info->data.ffc_jeita_temp_above_t3_cv = val;
	else {
		chr_err("use default FFC_JEITA_TEMP_ABOVE_T3_CV:%d\n",
			FFC_JEITA_TEMP_ABOVE_T3_CV);
		info->data.ffc_jeita_temp_above_t3_cv = FFC_JEITA_TEMP_ABOVE_T3_CV;
	}

	if (of_property_read_u32(np, "ffc_jeita_temp_t0_to_t3_cv", &val) >= 0)
		info->data.ffc_jeita_temp_t0_to_t3_cv = val;
	else {
		chr_err("use default FFC_JEITA_TEMP_T0_TO_T3_CV:%d\n",
			FFC_JEITA_TEMP_T0_TO_T3_CV);
		info->data.ffc_jeita_temp_t0_to_t3_cv = FFC_JEITA_TEMP_T0_TO_T3_CV;
	}

	if (of_property_read_u32(np, "ffc_jeita_temp_t0_to_t2_0_4420mv_cur", &val) >= 0)
		info->data.ffc_jeita_temp_t0_to_t2_0_4420mv_cur = val;
	else {
		chr_err("use default FFC_JEITA_TEMP_T0_TO_T2_0_4420MV_CUR:%d\n",
			FFC_JEITA_TEMP_T0_TO_T2_0_4420MV_CUR);
		info->data.ffc_jeita_temp_t0_to_t2_0_4420mv_cur = FFC_JEITA_TEMP_T0_TO_T2_0_4420MV_CUR;
	}

	if (of_property_read_u32(np, "ffc_jeita_temp_t0_to_t2_4420_4480mv_cur", &val) >= 0)
		info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur = val;
	else {
		chr_err("use default FFC_JEITA_TEMP_T0_TO_T2_4420_4480MV_CUR:%d\n",
			FFC_JEITA_TEMP_T0_TO_T2_4420_4480MV_CUR);
		info->data.ffc_jeita_temp_t0_to_t2_4420_4480mv_cur = FFC_JEITA_TEMP_T0_TO_T2_4420_4480MV_CUR;
	}

	if (of_property_read_u32(np, "ffc_jeita_temp_t0_to_t2_cutoff_cur", &val) >= 0)
		info->data.ffc_jeita_temp_t0_to_t2_cutoff_cur = val;
	else {
		chr_err("use default FFC_JEITA_TEMP_T0_T2_CUTOFF_CUR:%d\n",
			FFC_JEITA_TEMP_T0_T2_CUTOFF_CUR);
		info->data.ffc_jeita_temp_t0_to_t2_cutoff_cur = FFC_JEITA_TEMP_T0_T2_CUTOFF_CUR;
	}

	if (of_property_read_u32(np, "ffc_jeita_temp_t2_to_t3_cutoff_cur", &val) >= 0)
		info->data.ffc_jeita_temp_t2_to_t3_cutoff_cur = val;
	else {
		chr_err("use default FFC_JEITA_TEMP_T2_T3_CUTOFF_CUR:%d\n",
			FFC_JEITA_TEMP_T2_T3_CUTOFF_CUR);
		info->data.ffc_jeita_temp_t2_to_t3_cutoff_cur = FFC_JEITA_TEMP_T2_T3_CUTOFF_CUR;
	}

	if (of_property_read_u32(np, "ffc_jeita_temp_t3_to_t4_cutoff_cur", &val) >= 0)
		info->data.ffc_jeita_temp_t3_to_t4_cutoff_cur = val;
	else {
		chr_err("use default FFC_JEITA_TEMP_T3_T4_CUTOFF_CUR:%d\n",
			FFC_JEITA_TEMP_T3_T4_CUTOFF_CUR);
		info->data.ffc_jeita_temp_t3_to_t4_cutoff_cur = FFC_JEITA_TEMP_T3_T4_CUTOFF_CUR;
	}
/* Spruce code for OSPURCET-8 by zhangjb18 at 2022.11.29 end */
...
/* Spruce code for OSPURCET-3872 by yeyz5 at 2023.03.20 start */
static ssize_t show_Quick_Charge(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	chr_err("[%s]: %d\n", __func__, is_quick_charge);
	return sprintf(buf, "%d\n", is_quick_charge);
}
/* Spruce code for OSPURCET-3872 by yeyz5 at 2023.03.20 end */

static DEVICE_ATTR(Quick_Charge, 0644, show_Quick_Charge, NULL);

/* Spruce code for OSPURCET-2079 by yeyz5 at 2023.02.27 start */
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret = 0;
	gauge_psy = power_supply_get_by_name("battery");
	if (!IS_ERR_OR_NULL(gauge_psy)) {
...
static DEVICE_ATTR(Battery_Tempture, 0644, show_Battery_Tempture, NULL);
...
static DEVICE_ATTR(Battery_Present, 0644, show_Battery_Present, NULL);
/* Spruce code for OSPURCET-2079 by yeyz5 at 2023.02.27 end */
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	/* Spruce code for OSPURCET-3872 by yeyz5 at 2023.03.20 start */
	/* Quick charge */
	ret = device_create_file(&(pdev->dev), &dev_attr_Quick_Charge);
	if (ret)
		goto _out;
	/* Spruce code for OSPURCET-3872 by yeyz5 at 2023.03.20 end */
