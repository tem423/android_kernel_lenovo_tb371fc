...
#include "sc854x_charge_pump_reg.h"
...
/* Spruce code for OSPURCET-1426 by wumz4 at 2023.03.06 start */
static int sc854x_set_busucp_dg(struct sc854x *sc, int time)
{
    u8 val;
...
/* Spruce code for OSPURCET-1426 by wumz4 at 2023.03.06 end */
...
static void sc854x_dump_reg(struct sc854x *sc)
{
	int i = 0;
	u8 reg = 0;
	int ret = 0;
...
int sc854x_hw_chipid_detect(struct i2c_client *client)
{
	int ret = 0;

    ret = i2c_smbus_read_byte_data(client, SC854X_REG_36);
	if(ret < 0) {
		pr_err("i2c read fail:%s\n", __func__);
		return ret;
	} else if (ret != id_data[SC8545_ID] && ret != id_data[SC8546_ID] 
        && ret != id_data[SC8547_ID] && ret != id_data[SC8549_ID]) {
        pr_err("SC854X Device part ID reg 0x%02X\n", ret);
        return -ENOMEM;
    }

    pr_info("SC854X Device part ID reg 0x%02X\n", ret);
...
static int sc854x_parse_dt(struct sc854x *sc, struct device *dev)
{
    ...
    ret = of_property_read_u32(np, "sc,mode",
            &sc->mode);
    if (ret) {
    ...
    ret = sc854x_set_work_mode(sc, sc->mode);
    if (ret) {
        return ret;
    }
...
/* Spruce code for OSPURCET-1426 by wumz4 at 2023.03.06 end */
    ...
    /* Spruce code for OSPURCET-2786 by zhongxc7 at 2023.03.08 start */
    sc854x_enable_adc(sc, false);
    /* Spruce code for OSPURCET-2786 by zhongxc7 at 2023.03.08 end */
                    ...
                    const union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_ADC_DIS:
        if (val->intval){
            sc854x_enable_adc(sc, false);
            sc_err("%s plug out sc854x disable adc!\n",__func__);
        }else{
            sc854x_enable_adc(sc, true);
            sc_err("%s plug in sc854x enable adc!\n",__func__);
        }
        break;
    /* Spruce code for OSPURCET-2786 by zhongxc7 at 2023.03.08 end */
    default:
...
int sc854x_charger_probe(struct i2c_client *client,
...
int sc854x_suspend(struct device *dev)
{
...
int sc854x_suspend_noirq(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct sc854x *sc = i2c_get_clientdata(client);
...
int sc854x_resume(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct sc854x *sc = i2c_get_clientdata(client);
...
int sc854x_charger_remove(struct i2c_client *client)
{
    struct sc854x *sc = i2c_get_clientdata(client);
...
void sc854x_charger_shutdown(struct i2c_client *client)
{
/* Spruce code for OSPURCET-599 by zhangjb18 at 2022.12.06 start */
    ...
    ret = sc854x_enable_charge(sc, false);
    if (ret) {
        sc_err("Failed to disable charge\n");
    }

    ret = sc854x_enable_adc(sc, false);
    if (ret) {
        sc_err("Failed to disable adc\n");
    }
    pr_info("sc854x_charger_shutdown\n");
