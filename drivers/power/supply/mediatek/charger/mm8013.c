...
static int mm8013_write_volt(struct mm8013_chip *chip, int req)
{
	int ret = 0;
...
static int mm8013_checkdevice(struct mm8013_chip *chip)
{
	...
	if (ret == 0x0102) {
		ret = 2;
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 start */
		hardinfo_set_vendor_chginfo(HRADINFO_GAUGE_INFO, SCUD);
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 end */
	} else if (ret == 0x0101) {
		ret = 1;
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 start */
		hardinfo_set_vendor_chginfo(HRADINFO_GAUGE_INFO, SUNWODA);
		/* Spruce code for OSPURCET-3 by zhangjb18 at 2022.11.26 end */
	} else {
		ret = 0;
	}
...
/* Spruce code for OSPURCET-344 by zhangjb18 at 2022.12.08 start */
int mm8013_set_info(enum power_supply_property info_type, int val)
{
	int ret;
	...
	switch (info_type) {
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = mm8013_write_volt(_chip, val);
		break;
	default:
		ret = -1;
		break;
...
static void mm8013_battery_update_work(struct work_struct *work)
{
	...
	if(ret > 0) {
		...
		ret = mm8013_read_reg(chip->client, REG_CHARGEVOLT);
		if (ret > 0) {
			dev_info(&chip->client->dev, "[%s] get bm cv %d!\n", __func__, ret);
		}
	}
