...
static int ktz8866_backlight_conf(struct ktz8866 *bd)
{
	...
	if (!g_ktz8866_led.panel_id) {
		...
		dev_warn(&bd->client->dev,
			 "ktz8866 BOE Current reading 0x%02x is 0x%02x\n",
...
static int parse_dt(struct device *dev, struct ktz8866_platform_data *pdata)
{
	...
	pdata->panelid_gpio =
		of_get_named_gpio_flags(np, "ktz8866,panelid-gpio", 0, NULL);
			 ...
			 const struct i2c_device_id *id)
{
	...
	g_ktz8866_led.HBMenable = of_property_read_bool(
		(&client->dev)->of_node, "ktz8866,backlight-HBM-enable");
	...
	backlight_conf_disable = of_property_read_bool(
		(&client->dev)->of_node, "ktz8866,backlight-conf-disable");
