...
/*Spruce code for OSPURCET-1219 by zhuhao at 2023/2/9 start*/
bool mipi_ready = true;
bool mipi_delay = false;
/*Spruce code for OSPURCET-1219 by zhuhao at 2023/2/9 end*/
...
/*Spruce code for OSPURCET-1219 by zhuhao at 2023/2/9 start*/
	...
	if (dsi->ext && dsi->ext->params && dsi->ext->params->te_gpio) {
		params = dsi->ext->params;
	} else /* can't find panel ext information, stop esd read */
		return 0;

/*	lcd_te_gpio = devm_gpiod_get(dev, "lcd_te", GPIOD_OUT_HIGH);
	if (IS_ERR(lcd_te_gpio)) {
		dev_info(dev, "%s: cannot get lcd_te-gpios %ld\n",
				__func__, PTR_ERR(lcd_te_gpio));
		return PTR_ERR(lcd_te_gpio);
	}
*/
	/*Spruce code for OSPURCET-1219 by zhangkx10 at 2023/2/20 start*/
	if(gpiod_get_value(params->te_gpio)) {
		DDPPR_ERR("[DSI]esd cmp fail!\n");
		ret = -1;
	} else {
		DDPPR_ERR("[DSI]esd cmp OK!\n");
		ret = 0;
	}
		...
		struct mtk_dsi *dsi)
{
	...
	//spruce code for OSPURCET-2765 by zhouht5 at 20230315 start
	bw_base = bw_base * 10/8;
	//spruce code for OSPURCET-2765 by zhouht5 at 20230315 end
	return bw_base;
}
