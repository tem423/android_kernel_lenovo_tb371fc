...
//#include "ext_wd_drv.h"   //to-do
//#include "mtk_wdt.h" //to-do
...
static int __mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	int i;
#if IS_ENABLED(CONFIG_MTK_DEVINFO)
	const char *efuse_field[EFUSE_FIELD_CNT] = {"segment_table"};
	struct nvmem_cell *efuse_cell[EFUSE_FIELD_CNT];
	unsigned int *efuse_buf[EFUSE_FIELD_CNT];
	size_t efuse_len[EFUSE_FIELD_CNT];
	int j;
#endif
	...
	for (j = 0; j < EFUSE_FIELD_CNT; j++) {
		...
		memset(efuse_len, 0, sizeof(efuse_len));
		efuse_buf[j] =	(unsigned int *)nvmem_cell_read(efuse_cell[j], &efuse_len[j]);
	...
	g_efuse_speed_bound_id = ((*efuse_buf[0]) & 0x0F000000);
	g_efuse_turbo_id = ((*efuse_buf[0]) & 0x00000008);
#else
