...
static irqreturn_t hall_interrupt_handler(int irq, void *dev)
{
	...
	hall_dbg("hall_interrupt_handler enter\n");
	...
	if (value) {
		...
		//dev_dbg(&data->hall_dev->dev, "hall switch near\n");
		hall_disable_fp_pk = true;
		hall_dbg("hall switch near\n");
	}
...
static ssize_t hall_int_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	/* place holder for future use */
	return -EPERM;
}

static ssize_t hall_int_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0;
	struct hall_data *data = dev_get_drvdata(dev);
	num_read_chars = snprintf(buf, 25,"%d\n",gpio_get_value(data->gpio));
	hall_dbg("hall int show gpio %d \n",gpio_get_value(data->gpio));
	return num_read_chars;
}

static DEVICE_ATTR(hall_int_gpio, S_IRUGO|S_IWUSR, hall_int_show, hall_int_store);
...
static int hall_driver_probe(struct platform_device *dev)
{
	...
	/* test mode node */
	err = sysfs_create_group(&dev->dev.kobj, &hall_attribute_group);
	if (0 != err) {
		hall_dbg("%s() - ERROR: sysfs_create_group() failed.\n", __func__);
		sysfs_remove_group(&dev->dev.kobj, &hall_attribute_group);
		return -EIO;
	}
	else {
		pr_info("hall:%s() - sysfs_create_group() succeeded.\n",__func__);
	}
	//err=sysfs_create_link(NULL,&dev->dev.kobj,"hall_sensor");
	/* test mode node */
	return 0;
