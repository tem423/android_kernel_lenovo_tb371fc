...
//	__pm_wakeup_event(&fp_wakesrc, WAKELOCK_HOLD_TIME);
	...
	if (gf_dev->async)
        ...
        pr_info("Hall sensor disable fignerprint!\n");
}
	return IRQ_HANDLED;
}
...
//power-on
	gf_dev->vdd_supply = devm_regulator_get(dev, "vdd");
	if (IS_ERR(gf_dev->vdd_supply)) {
		pr_err("failed to get vdd!\n");
		goto error_input;
	}
	//set_power
	//ret =regulator_set_voltage(gf_dev->vdd_supply, 3000000, 3000000);
	//if(ret !=0){
		//ts_err("set: Failed to set avdd: %d\n", ret);
	//}
	regulator_enable(gf_dev->vdd_supply);
...
//	wakeup_source_init(&fp_wakesrc, "fp_wakesrc");
	wakeup_source_register(dev, "fp_wakesrc");
