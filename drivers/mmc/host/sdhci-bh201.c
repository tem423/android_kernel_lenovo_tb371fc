...
int sdhci_bht_sdr104_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	...
	do {
		...
		spin_lock_irqsave(&vendor_host->lock, flags);
		usleep_range(2000, 2200);//delay 1ms for CMD19
...
int sdhci_bht_sdr104_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	...
	pr_debug("%s: Enter %s  -- 2ms delay 20210806\n", mmc_hostname(mmc), __func__);
...
retry:
	...
	do {
		...
		mmc_wait_for_req(mmc, &mrq);
		usleep_range(2000, 2200);//delay 2ms for CMD19
...
#define GGC_SW_SELB_TUNING_LOOP 3
	...
	vendor_host->sdr50_notuning_sela_inject_flag = 0;
