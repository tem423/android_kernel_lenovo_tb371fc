...
<<<<<<< HEAD
	 ...
	 */
	/* TB371FC p97: mi_cfg.bl_enable stays false from parse on this panel
	...
	if (!dsi_panel_initialized(panel)) {
		panel->bl_config.bl_level = (u32)bl_temp;
		DSI_DEBUG("skip backlight write: panel not ready, lvl=%u",
			(u32)bl_temp);
		rc = 0;
		goto error;
	}
